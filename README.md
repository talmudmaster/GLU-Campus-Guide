...
> **① 多校区切换已经大致实现（可能存在少量bug），需要的可以通过下面的联系方式私聊**  
<br>
> **② 实在不会搞的，可以通过下面的联系方式私聊**  
<br>
> **③ 可自由修改该项目，重构后端（如springboot），重构前端（如uni、鸿蒙），甚至可以改成web、app（毕竟地图数据的处理逻辑是相似的）**  
<br>
...
<br>
> ③ 小程序通过微信认证（个人30/公司300，认证一年且不保证通过），才能被搜索到和被分享给其他人  
<br>
...
- 实现 多校区切换（**已经实现，但可能存在少量bug，需要的可以联系**） ✔️
- 实现 全景图 动态管理
- 实现 校车路线、浏览路线展示
- 引入 AI大语言模型对话
